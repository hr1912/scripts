...
# Modified by: Hang Ruan, Novogene Bioinformatics Technology Co.Ltd
# Modified on: $Date: 2013/02/01 
...
my $known_genome_size ; # if genome size is previously known or estimated 
my $diploid; # flag of a diploidy reference
...
my $usage = "Usage: assemblathon_stats.pl <reference_file> <assembly_file>
options:
	-gsize <int> specify genome size if a reference is not present
	-diploid     if reference contains two similar haploid sequences 
...
#add by Hang Ruan,  reference basic statistics
if ($ref) {
...
##########################################
#    reference sequence basic statistics
##########################################

sub reference_statistics{
	...
	while(my $entry = $fasta->nextEntry){
		...
		$seq_count++;
		$known_genome_size += $length;
		$N_count += ($seq =~ tr/N/N/);
	}
	
	print "<-- Information for reference \'$ref\' -->\n";	
	printf "\nReference includes %2d sequences, %10d total length, %8.2f N percent\n", $seq_count, $known_genome_size,  $N_count * 100 / $known_genome_size;
	$known_genome_size /= $seq_count if ($diploid);
}	
